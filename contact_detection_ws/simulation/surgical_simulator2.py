...
class SurgicalContactSimulator:
    ...
    def _add_anatomical_structures(self):
        # FIXED: Tissue with proper anchoring
        ...
        self.anatomy_objects['tissue'] = p.createMultiBody(
            baseMass=0.1,  # Increased mass for stability
            baseCollisionShapeIndex=tissue_collision,
            baseVisualShapeIndex=tissue_visual,
            basePosition=[0.6, 0, 0.11]
        )

        # Higher friction and damping to prevent sliding
        p.changeDynamics(self.anatomy_objects['tissue'], -1, 
                         lateralFriction=2.0, rollingFriction=0.5, restitution=0.1, 
                         linearDamping=0.9, angularDamping=0.9, 
                         contactStiffness=1000, contactDamping=100)
        
        # FIXED: Organ positioned for better contact
        ...
        # FIXED: Bone positioned for better contact
        bone_collision = p.createCollisionShape(p.GEOM_CYLINDER, radius=0.02, height=0.1)
        bone_visual = p.createVisualShape(p.GEOM_CYLINDER, radius=0.02, length=0.1, rgbaColor=[0.9, 0.9, 0.8, 1])
        self.anatomy_objects['bone'] = p.createMultiBody(
            baseMass=0.2,
            baseCollisionShapeIndex=bone_collision,
            baseVisualShapeIndex=bone_visual,
            basePosition=[0.7, -0.1, 0.15]
        )
        ...
        # FIXED: Add constraints to prevent objects from moving too much
    ...
    def _add_stability_constraints(self):
        """Add constraints to keep anatomical objects stable"""
        # Soft constraint for tissue (allows some movement but prevents sliding off)
        ...
        p.changeConstraint(tissue_constraint, maxForce=5.0)  # Soft constraint
        self.constraints['tissue'] = tissue_constraint

        # Constraint for organ
        ...
        bone_constraint = p.createConstraint(
            ...
            jointType=p.JOINT_POINT2POINT,
            jointAxis=[0, 0, 0],
            parentFramePosition=[0.1, -0.1, 0.09],
            childFramePosition=[0, 0, 0]
        )
        p.changeConstraint(bone_constraint, maxForce=15.0)
    ...
    def _bone_drilling_scenario(self):
        # FIXED: New scenario to contact the bone
        target_positions = [
            [0.4, 0, 0.4],      # start
            [0.7, -0.1, 0.3],   # move over bone
            [0.7, -0.1, 0.22],  # approach bone
            [0.7, -0.1, 0.19],  # contact bone
            [0.7, -0.08, 0.19], # drilling motion
            [0.7, -0.12, 0.19], # continue drilling
            [0.7, -0.1, 0.19],  # center
            [0.7, -0.1, 0.25],  # retract
        ]
