...
class SurgicalContactSimulator:
    ...
    def _add_surgical_instruments(self):
        grasper_collision = p.createCollisionShape(p.GEOM_CYLINDER, radius=0.002, height=0.1)
        grasper_visual = p.createVisualShape(p.GEOM_CYLINDER, radius=0.002, length=0.1, rgbaColor=[0.8, 0.8, 0.8, 1])
        ...
        scissors_collision = p.createCollisionShape(p.GEOM_BOX, halfExtents=[0.001, 0.02, 0.05])
        scissors_visual = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.001, 0.02, 0.05], rgbaColor=[0.7, 0.7, 0.9, 1])
    ...
    def _add_anatomical_structures(self):
        tissue_collision = p.createCollisionShape(p.GEOM_BOX, halfExtents=[0.08, 0.05, 0.03])
        tissue_visual = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.08, 0.05, 0.03], rgbaColor=[0.8, 0.4, 0.4, 0.9])
        self.anatomy_objects['tissue'] = p.createMultiBody(
            baseMass = 0.2,
            baseCollisionShapeIndex = tissue_collision,
            baseVisualShapeIndex = tissue_visual,
            basePosition=[0.4, 0, 0.15]
        )
        ...
        bone_collision = p.createCollisionShape(p.GEOM_CYLINDER, radius=0.02, height=0.15)
        bone_visual = p.createVisualShape(p.GEOM_CYLINDER, radius=0.02, length=0.15, rgbaColor=[0.9, 0.9, 0.8, 1])
        self.anatomy_objects['bone'] = p.createMultiBody(
            baseMass = 0.5,
            baseCollisionShapeIndex = bone_collision,
            baseVisualShapeIndex = bone_visual,
            basePosition = [0.6, -0.2, 0.3]
        )

        p.changeDynamics(self.anatomy_objects['bone'], -1,
                         lateralFriction=0.6, restitution=0.2,
    ...
    def _tissue_manipulation_scenario(self):
        target_positions = [
            [0.3, 0, 0.25],   # approach
            [0.4, 0, 0.18],   # contact tissue
            [0.4, 0.05, 0.18],   # manipulate
            [0.4, -0.05, 0.18],   # continue manipulation
            [0.3, 0, 0.25],   # retract
        ]
    ...
    def _suturing_scenario(self):
        ...
        for i in range(5):
            x = 0.35 + i * 0.02
            y = 0.02 * math.sin(i*math.pi / 2)
            z = 0.16 + 0.01 * math.cos(i * math.pi / 2)
    ...
    def _execute_waypoint_motion(self, waypoints: List[List[float]], speed_factor: float = 1.0):
        ...
        for waypoint in waypoints:
            ...
            for step in range(int(100*speed_factor)):
    ...
    def collect_surgical_data(self, num_scenarios: int = 5, data_file: str = "surgical_data.csv"):
        ...
        with open(filepath, 'w', newline='') as csvfile:
            ...
            num_joints = len(all_data[0]['positions'])
            fieldnames = []
